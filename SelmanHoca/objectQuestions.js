...
const results = { Java: 10, Ruby: 80, Python: 95, ASM: 85, Haskel: 90 };
const bigger60 = {}; 
let biggerOrder = [];
for (let result in results) {
  if (results[result] >= 60) {
    biggerOrder.push(results[result]);  //60 dan büyük olanların sadece notlarını bir array içine kaydediyor
    bigger60[results[result]] = result; //60 dan büyük olanları key ve value değerlerini ters çevirerek yeni objeye kaydediyor.
  }
}
// chaining metoduyla önce notlar büyükten küçüğe sıralanıyor, sonra map fonksiyonuyla notlar nesne içindeki value değerleriyle değiştiriliyor 
biggerOrder = biggerOrder.sort((a, b) => b - a).map((item) => bigger60[item]);
